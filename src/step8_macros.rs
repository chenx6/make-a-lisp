...
fn is_macro_call(ast: &MalType, env: &mut env::Env) -> bool {
    if let MalType::List(ref l, _) = ast {
        if let Some(MalType::Sym(_)) = l.get(0) {
            if let Some(MalType::MalFunc { is_macro, .. }) = env.get(&l[0]) {
                return is_macro;
            }
        }
    }
...
fn macroexpand(ast: &MalType, env: &mut env::Env) -> Result<(bool, MalType), MalErr> {
    let mut fast = ast.clone();
    let mut expanded = false;
    while is_macro_call(&fast, env) {
        match fast {
            MalType::List(l, _) if l.len() > 0 => {
                let last = match env.get(&l[0]) {
                    Some(v) => v,
                    None => return Err(error_msg("macroexpand cannot found this symbol")),
                };
                fast = apply(&last, l[1..].to_vec())?;
                expanded = true;
            }
            _ => return Err(error_msg("macroexpand argument type error")),
        }
    }
    Ok((expanded, fast))
}

fn EVAL(val: &MalType, env: &mut env::Env) -> Result<MalType, MalErr> {
    ...
    loop {
        match ast {
            ...
            MalType::List(ref uneval_lst, _) => {
                let expanded = macroexpand(&ast, &mut env)?;
                match expanded.0 {
                    // (true, MalType::List(_, _)) => {
                    true => {
                        ast = expanded.1;
                        continue;
                    }
                    // (true, _) => return eval_ast(&ast, &mut env),
                    false => (),
                }
                match &uneval_lst[0] {
                    ...
                    MalType::Sym(s) if s == "defmacro!" => {
                        if uneval_lst.len() != 3 {
                            return Err(error_msg("defmacro! len() != 3"));
                        }
                        let evaled = EVAL(&uneval_lst[2], &mut env)?;
                        match evaled {
                            MalType::MalFunc {
                                ...
                                return match env.set(
                                    ...
                                    Some(v) => Ok(v),
                                    None => Err(error_msg("defmacro! set env fail")),
                                }
                            }
                            _ => {
                                return Err(error_msg("defmacro! second argument is not function"))
                            }
                        };
                    }
                    MalType::Sym(s) if s == "macroexpand" => {
                        return Ok(macroexpand(&uneval_lst[1], &mut env)?.1);
                    }
                    // _default_
                    _ => {
                            ...
                            match l.get(0) {
                                ...
                                _ => {
                                    return Err(error_msg(
                                        "_default_ first argument is not function",
                                    ));
                                }
                            }
                            continue;
...
fn main() {
    ...
    for f in [
        ...
        let f = match read_str(&f.to_string()) {
            Some(v) => v,
            None => return,
        };
